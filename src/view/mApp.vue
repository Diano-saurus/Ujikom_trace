  ...
  >
    ...
    <div class="flex items-center gap-2">
      ...
      <el-dialog
        ...
        center
      >
        <el-form class="demo-form-inline" @submit.prevent="createData">
...
const tableHeaders = ref([
  { prop: "No", label: "No." },
  { prop: "code", label: "App Code" },
  { prop: "name", label: "App Name" },
...
const getData = async () => {
  try {
    const response = await axios.get(
      "https://b8ae-36-72-54-195.ngrok-free.app/api/master-app",
      {
        headers: {
    ...
    settingsTable.value = dataArray.map((app, index) => ({
      No: index + 1,
      code: app.code || app.id || "Unknown",
      name: app.name || app.app_name || "Unknown Name",
    }));
  } catch (error) {
...
const createData = async () => {
  ...
  try {
    await axios.post(
      "https://b8ae-36-72-54-195.ngrok-free.app/api/master-app/add",
      {
        app_name: name.value,
      }
    );
...
const deleteItem = async (code) => {
  try {
    await axios.delete(
      `https://b8ae-36-72-54-195.ngrok-free.app/api/master-app/delete/${code}`
    );
    Swal.fire({ icon: "success", title: "Data Terhapus!", timer: 1000 });
    getData();
  } catch (error) {
...
const updateItem = async (code, newName) => {
  try {
    await axios.put(
      `https://b8ae-36-72-54-195.ngrok-free.app/api/master-app/edit/${code}`,
      {
        app_name: newName,
      }
    );
